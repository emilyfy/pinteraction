...
CBlobTracking::CBlobTracking(int hMinMax[2], int sMinMax[2], int vMinMax[2]
	, int frameWidth, int frameHeight, int maxObjects, int minObjectArea, int maxObjectArea
	...
	if ((int)hierarchy.size()>0 && (int)hierarchy.size()<m_maxObjects)
	{
		...
		while (index >= 0)
		{
			...
			if (area >m_minObjectArea && area<m_maxObjectArea && area > holdArea)
			{
				...
				putText(original, to_string(c.point.x) + "," + to_string(c.point.y), Point(c.point.x, c.point.y + 30), 1, 1, Scalar(0, 0, 255), 2);

				cout << c.point.x << ", " << c.point.y << endl;

				int xco = floor(c.point.x / 64); //x-coordinate (column number) of the pin
				int yco = floor(c.point.y / 48); //y-coordinate (row number) of the pin
